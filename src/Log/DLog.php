...
class DLog
{
    ...
    public static function getInstance($app = null, $logType = null)
    {
        ...
        if (empty(self::$arrInstance[$app])) {
            ...
            //get log format
            if (isset(DLogConfig::$format)) {
                $format = DLogConfig::$format;
            } else {
                $format = self::DEFAULT_FORMAT;
            }

            if (isset(DLogConfig::$format_wf)) {
                $format_wf = DLogConfig::$format_wf;
            } else {
                $format_wf = $format;
            }

            $log_conf = array(
                'level'       => intval(DLogConfig::$level),
                'auto_rotate' => (DLogConfig::$auto_rotate == '1'),
                'log_file'    => $log_file,
                'format'      => $format,
                'format_wf'   => $format_wf,
            );
